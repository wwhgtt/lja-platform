...
.controller("getSiteInfo",function(
	...
	$scope.getSite={long:"",lat:"",distcine:""};//先设置一个存放页面输入数据的对象；其实就是一个json数组；
	$scope.getSiteInfo = function(){  // 调用html页面里面的ng-submit函数；
        ...
        var distcine = $scope.getSite.distcine;
		$GetSiteInfo.getSiteInfo(long,lat,distcine,function(err,result){//err == null           $login是之前定义的服务  login是一个json对象  是服务定义后的内容（存放）  err必须为第一个参数
			//function
